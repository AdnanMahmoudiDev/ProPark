...
<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between {{ $align }}">
            <div>
                <h2 class="text-2xl font-bold tracking-tight text-white">
                    {{ __('profile_settings_title') }}
                </h2>
                <p class="mt-1 text-sm text-gray-400">
                    {{ __('profile_settings_desc') }}
                </p>
            </div>
            ...
            >
                <span>{{ __('profile_back_to_dashboard') }}</span>
                <svg xmlns="http://www.w3.org/2000/svg"
                     class="h-4 w-4 transition duration-300 {{ $isRtl ? 'group-hover:translate-x-1' : 'group-hover:-translate-x-1 rotate-180' }}"
                     fill="none"
                     viewBox="0 0 24 24"
                     stroke="currentColor">
