...
<section class="space-y-6" dir="{{ $isRtl ? 'rtl' : 'ltr' }}">
    <header>
        <div class="flex items-start gap-3 {{ $align }}">
            <div class="flex h-11 w-11 items-center justify-center rounded-2xl border border-red-500/20 bg-red-500/10 text-red-300 shadow-lg shadow-red-950/20">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M12 9v4m0 4h.01M10.29 3.86l-7.5 13A2 2 0 004.5 20h15a2 2 0 001.71-3.14l-7.5-13a2 2 0 00-3.42 0z" />
                </svg>
            </div>
        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div class="{{ $align }}">
                <h3 class="text-sm font-semibold text-red-200">
                    {{ __('delete_account_irreversible_title') }}
                </h3>
                <p class="mt-1 text-sm text-gray-400">
                    {{ __('delete_account_irreversible_desc') }}
                </p>
            </div>
    ...
    <x-modal name="confirm-user-deletion" :show="$errors->userDeletion->isNotEmpty()" focusable>
        <form method="post" action="{{ route('profile.destroy') }}" class="rounded-3xl border border-white/5 bg-[#0f1420] p-6 shadow-2xl shadow-black/30" dir="{{ $isRtl ? 'rtl' : 'ltr' }}">
            @csrf
            @method('delete')
            ...
            <div class="flex items-start gap-3 {{ $align }}">
                ...
                <div>
                    <h2 class="text-lg font-bold tracking-tight text-white">
                        {{ __('delete_account_confirm_title') }}
                    </h2>

                    <p class="mt-1 text-sm leading-6 text-gray-400">
                        {{ __('delete_account_confirm_desc') }}
                    </p>
                </div>
            </div>
            ...
            <div class="mt-6">
                ...
                <x-input-error :messages="$errors->userDeletion->get('password')" class="mt-2 {{ $align }}" />
            </div>
